...
public class DistCpUtils {
  ...
  public static long getFileSize(Path path, Configuration configuration)
    ...
    if (logger.isDebugEnabled())
      logger.debug("Retrieving file size for: " + path);
  ...
  public static LinkedList<CopyListingFileStatus> toCopyListingFileStatus(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("toCopyListing: " + fileStatus + " chunkSize: "
    ...
    if ((blocksPerChunk > 0) &&
      ...
      if (numBlocks <= blocksPerChunk) {
        ...
        while (i < numBlocks) {
          ...
          if (curLength > 0) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug("  add file chunk " + clfs1);
            }
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("  add file/dir " + clfs);
      }
      copyListingFileStatus.add(clfs);
    }
  ...
  public static CopyListingFileStatus toCopyListingFileStatusHelper(
    ...
    logger.info("preserveRawXAttrs " + preserveRawXAttrs);
    if (preserveXAttrs || preserveRawXAttrs) {
    ...
    logger.info("======= toCopyListingFileStatusHelper end! =========");
    return copyListingFileStatus;
  }
