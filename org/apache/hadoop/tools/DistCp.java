...
public class DistCp extends Configured implements Tool {
  ...
  private void prepareFileListing(Job job) throws Exception {
    if (inputOptions.shouldUseSnapshotDiff()) {
      ...
      // in regular way.
      createInputFileListing(job);// 创建 fileList.seq 文件
    }
  }
  ...
  public Job createAndSubmitJob() throws Exception {
    ...
    try {
      ...
      prepareFileListing(job);// 创建 filelist.seq 文件
      job.submit();
      submitted = true;
    } finally {
      if (!submitted) {
  ...
  private void setTargetPathExists() throws IOException {
    ...
    inputOptions.setTargetPathExists(targetExists);// 设置 target 目标端是否存在此目录
    getConf().setBoolean(DistCpConstants.CONF_LABEL_TARGET_PATH_EXISTS, 
        targetExists);// DistCpConstants 存储文件的状态信息
  }
  ...
  private Job createJob() throws IOException {
    ...
    job.setInputFormatClass(DistCpUtils.getStrategy(getConf(), inputOptions));// inputOptions 存储一些写入 Map 的参数
    job.setJarByClass(CopyMapper.class); // 设置执行 map 的类
    configureOutputFormat(job); // 输出到哪个 job
  ...
  private Path createMetaFolderPath() throws Exception {
    ...
    configuration.set(DistCpConstants.CONF_LABEL_META_FOLDER, metaFolderPath.toString());  // 设置distcp 文件的存储位置
  ...
  public static void main(String argv[]) {
    int exitCode;
    argv = new String[]{"-update","-append","hdfs://192.168.46.11:9000/once11","hdfs://192.168.48.11:9000/once11/bb"};
    try {
